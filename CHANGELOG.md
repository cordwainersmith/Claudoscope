...
- **Session lifecycle notifications (opt-in).** Settings > Notifications installs a Claude Code `Notification` hook and delivers native macOS notifications for three independently toggleable events (all on once the feature is enabled): a real block waiting on you (permission, plan, or MCP prompt), the idle "Claude is waiting for your input" prompt, and a session that finished after a long run (10+ minutes active, then 3+ minutes quiet). Includes per-project mute, daily quiet hours, and a sound toggle. Each notification is labeled with the project/repo folder name (a `/rename` or slug title appears as "Title (folder)"), taken from the hook payload so it reads the same across any terminal. An existing hand-rolled `session-notify.sh` is detected, and its Notification and Stop entries are backed up and disabled on enable, then restored on disable, so banners are never duplicated.
