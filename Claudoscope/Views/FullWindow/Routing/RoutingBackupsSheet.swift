...
/// List of `~/.claude/.claudoscope-routing-backup-*` directories with size,
/// timestamp, restore (only enabled for the marker-current backup), and
/// delete per row. If more than 5 backups exist, surfaces a Prune to last 5
/// button.
struct RoutingBackupsSheet: View {
    ...
    private func reload() {
        ...
        for url in dirs {
            let timestamp = url.lastPathComponent
                .replacingOccurrences(of: RoutingStackInstaller.backupPrefix, with: "")
            let date = dateFormatter.date(from: timestamp)
            let size = InstallerFileOps.sizeOf(directory: url)
        ...
        let markerURL = claudeDirURL.appendingPathComponent(RoutingStackInstaller.markerFileName)
...
private struct RoutingBackupRowView: View {
