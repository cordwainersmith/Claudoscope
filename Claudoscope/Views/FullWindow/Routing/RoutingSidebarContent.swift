...
// MARK: - Routing Sidebar Content

/// Two-item sidebar: "Overview" (default) and a collapsible "Lint Issues" group
/// scoped to RTG* lint results, with severity counts. Selecting Overview clears
...
struct RoutingSidebarContent: View {
    ...
    private var rtgResults: [LintResult] {
        let scoped = lintResults.filter { $0.checkId.rawValue.hasPrefix("RTG") }
    ...
    private var groupedBySeverity: [(label: String, severity: LintSeverity, items: [LintResult])] {
        let errors = rtgResults.filter { $0.severity == .error }
        let warnings = rtgResults.filter { $0.severity == .warning }
        let infos = rtgResults.filter { $0.severity == .info }
    ...
    private var lintIssuesGroup: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if lintIssuesExpanded {
                if rtgResults.isEmpty {
                    HStack(spacing: 6) {
                        Image(systemName: "checkmark.circle")
                            .font(.system(size: 12))
                            .foregroundStyle(.green)
                        Text("All routing checks pass")
                            .font(Typography.body)
    ...
    private var severityCountsView: some View {
        HStack(spacing: 4) {
            if rtgSummary.errorCount > 0 {
                countPill(count: rtgSummary.errorCount, color: colorForSeverity(.error))
            }
            if rtgSummary.warningCount > 0 {
                countPill(count: rtgSummary.warningCount, color: colorForSeverity(.warning))
            }
            if rtgSummary.infoCount > 0 {
                countPill(count: rtgSummary.infoCount, color: colorForSeverity(.info))
            }
            if rtgResults.isEmpty {
...
private struct RoutingIssueRow: View {
